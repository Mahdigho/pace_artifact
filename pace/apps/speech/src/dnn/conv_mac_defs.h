...
static inline void _prep_chunk_for_conv(fixed *src, uint16_t src_chn_size,
  ...
  uint16_t dst_increment = mat_mul_cols * strides.cols;
...
static inline void _gmm_chn(fixed *dst_chn, bool first_chns,
  ...
  if (!first_chns) {
    FRAM_MOVE(dst_chn, __lea_conv_dst, move_size);
    __add_q15(__lea_conv_tmp);
    FRAM_MOVE(__lea_conv_dst, dst_chn, move_size);
  } else {
    FRAM_MOVE(__lea_conv_tmp, dst_chn, move_size);
  }
}
